...
it('dữ liệu cũ (thiếu xp/badges/daily/stats) vẫn đọc được, giữ nguyên dữ liệu cũ', () => {
  const old = { phrases: { 'L:g1': { box: 2, wrong: 0, seen: 2, last: 1 } }, bestScores: { 'L:quiz': 80 }, missions: {}, streak: { count: 3, lastDay: '2026-09-21' } }
  localStorage.setItem(KEY, JSON.stringify(old))
  const p = loadProgress()
  expect(p.phrases).toEqual(old.phrases)
  expect(p.bestScores['L:quiz']).toBe(80)
  expect(p.streak.count).toBe(3)
  expect(p.xp).toBe(0)
  expect(p.badges).toEqual({})
  expect(p.daily).toEqual(emptyProgress().daily)
  expect(p.stats).toEqual(emptyProgress().stats)
})
it('trường mới lưu thiếu một phần → gộp với mặc định', () => {
  localStorage.setItem(KEY, JSON.stringify({ xp: 120, daily: { day: '2026-09-22', plays: 2 }, stats: { plays: 5 } }))
  const p = loadProgress()
  expect(p.xp).toBe(120)
  expect(p.daily.plays).toBe(2)
  expect(p.daily.games).toEqual([])
  expect(p.stats.plays).toBe(5)
  expect(p.stats.playedByTier).toEqual({})
